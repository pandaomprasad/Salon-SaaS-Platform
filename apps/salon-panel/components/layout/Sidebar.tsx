...
import {
  ...
  Scissors, CalendarClock, BarChart3, Bell, LogOut,
} from 'lucide-react'
...
const NAV_ITEMS: NavItem[] = [
  { page: 'dashboard',     label: 'Dashboard',     icon: LayoutDashboard, roles: ['owner', 'manager', 'staff'] },
  { page: 'bookings',      label: 'Bookings',       icon: CalendarDays,    roles: ['owner', 'manager', 'staff'] },
  { page: 'customers',     label: 'Customers',      icon: Users,           roles: ['owner', 'manager', 'staff'] },
  { page: 'services',      label: 'Services',       icon: Scissors,        roles: ['owner', 'manager']          },
  { page: 'schedule',      label: 'Schedule',       icon: CalendarClock,   roles: ['owner', 'manager']          },
  { page: 'reports',       label: 'Reports',        icon: BarChart3,       roles: ['owner']                     },
  { page: 'notifications', label: 'Notifications',  icon: Bell,            roles: ['owner', 'manager', 'staff'] },
]
...
export default function Sidebar({
  currentPage, role, name, email, initials, onNavigate, onLogout,
}: SidebarProps) {
  const unread       = NOTIFICATIONS.filter(n => !n.read).length
  const visibleItems = NAV_ITEMS.filter(item => item.roles.includes(role))

  return (
    <aside className="fixed inset-y-0 left-0 w-56 bg-ink flex flex-col z-40">
  ...
  ${active
    ? 'text-white bg-white/8 border-l-2 border-gold pl-[14px]'
    : 'text-silver/60 hover:text-silver hover:bg-white/5 border-l-2 border-transparent pl-[14px]'
  }
`}
            >
